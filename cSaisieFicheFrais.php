 ...
 */
 ...
 if((int)date("j")<10){
     ...
     if(obtenirLibelleMois(intval(substr($mois,4,2)))=="Jan"){

         $mois = sprintf("%04d%02d", date("Y")-1 ,date("m")-1);

     }else{

         $mois = sprintf("%04d%02d", date("Y"), date("m")-1);
...
?>
  ...
  <div id="contenu">
         ...
         <!-- Automatise l'entrée selon la date (si jour<10, rentre dans le mois précédent) -->

      <h2>Renseigner ma fiche de frais du mois de

      <?php

         echo obtenirLibelleMois(intval(substr($mois,4,2))) . " " . substr($mois,0,4); ?>
